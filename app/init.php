...
if(@check_var($APPConfig['AUTH']['INDEX']['PASSWORD'])){
	if(!@check_var($hfs4OSS_cookies['passwords']['index'])){
		$resultToSendBack['stat'] = 201;
		$resultToSendBack['msg'] = $APPConfig['AUTH']['INDEX']['FIRSTMET'];
		die(json_encode($resultToSendBack));
	}else{
		if($hfs4OSS_cookies['passwords']['index'] !== $APPConfig['AUTH']['INDEX']['PASSWORD']){
			$resultToSendBack['stat'] = 202;
			$resultToSendBack['msg'] = $APPConfig['AUTH']['INDEX']['IFWRONG'];
			die(json_encode($resultToSendBack));
		}
	}
}