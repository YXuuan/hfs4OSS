...
//获得上层目录名称
$last_floder_name = $explode_path[count($explode_path)-2] ;
...
//获得上层目录路径
for($i = 0; $i < count($explode_path)-2; $i++){	//拼接已拆分的数组
	$last_floder_path = $last_floder_path."/".$explode_path[$i] ;
}
$last_floder_path = substr($last_floder_path."/", 1) ;	//在末尾加入“/”并去除字符串开头的“/”
...
<html class="hb-loaded" lang="en">
	...
	<body class="index" id="root" screen_capture_injected="true" huaban_collector_injected="true">
		<div id="topbar">
			...
			<a id="backlink" href="https://github.com/YuXuan220/" target="_blank" title="Powered by YuXuan_0012 - https://github.com/YXuuan/">
				<div>Powered by</div>
				<div>YXuuan.</div>
			</a>
